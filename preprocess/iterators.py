...
class ImageNetWithPathIterator(Dataset):
    def __init__(self, config) -> None:
        self.image_paths = self._inspect_images(config.get("dataset_path", '../dataset'))
...
class ImageNetLatentIterator(Dataset):
    ...
    def collate(self, batch):
        number_of_tokens = random.choice([32, 64, 96, 128, 192, 256])
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(latent), torch.tensor(label), torch.tensor(pos), torch.tensor(mask)

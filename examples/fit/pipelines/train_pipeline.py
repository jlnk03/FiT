...
class NetworkWithLoss(nn.Module):
    def __init__(
        self,
        network: nn.Module,
    ...
    def vae_encode(self, x):
        if self.latent_input:
            ...
            image_latents = pt.autograd.enable_grad(self.vae.encode(x))
        image_latents = image_latents * self.scale_factor
        return image_latents.astype(pt.float16)
    ...
    def _cal_vb(self, model_output, model_var_values, x, x_t, t, mask=None):
        ...
        max_log = extract_into_tensor(pt.log(self.diffusion.betas), t, x_t.shape)
    ...
    def unpatchify(self, x):
        ...
        x = pt.reshape(x, (x.shape[0], nh, nw, p, p, c))
        x = pt.transpose(x, (0, 5, 1, 3, 2, 4))
        x = pt.reshape(x, (x.shape[0], c, nh * p, nw * p))
        return x
    ...
    def compute_loss(self, x: Tensor, y: Tensor, pos: Tensor, mask: Tensor) -> Tensor:
        ...
        x = self.unpatchify(x)
        t = pt.randint(0, self.diffusion.num_timesteps, (x.shape[0],))
        ...
        mask = self.unpatchify(pt.tile(mask[..., None], (1, 1, D)))
        vb = self._cal_vb(pt.autograd.no_grad(model_output), model_var_values, x, x_t, t, mask=mask)

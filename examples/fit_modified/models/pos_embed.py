...
def create_sinusoidal_positions(num_pos: int, dim: int) -> Tensor:
    ...
    sinusoid_inp = pt.cat((Tensor(sinusoid_inp, dtype=pt.float32), Tensor(sinusoid_inp, dtype=pt.float32)), axis=-1)
    return pt.cat((pt.sin(sinusoid_inp), pt.cos(sinusoid_inp)), axis=1)


def rotate_every_two(x: Tensor):
    x1 = x[:, :, :, ::2]
    x2 = x[:, :, :, 1::2]
    x = pt.stack((-x2, x1), axis=-1)
...
def apply_2d_rotary_pos(q: Tensor, k: Tensor, freqs_cis: Tensor) -> Tensor:
    ...
    sin_h, cos_h = pt.split(sincos_h, sincos_h.shape[-1] // 2, axis=-1)
    sin_w, cos_w = pt.split(sincos_w, sincos_w.shape[-1] // 2, axis=-1)
    ...
    q = pt.concat([q1, q2], axis=-1)
    k = pt.concat([k1, k2], axis=-1)
    return q, k

...
class FiTModule(L.LightningModule):
    def __init__(self, args):
        ...
        self.model = FiT_models[args.model](mask_ratio=0.75, decode_layer=4)
    ...
    def forward(self, x, y, pos, mask, t, enable_mask=True):
        return self.model(x, t=t, y=y, pos=pos, enable_mask=enable_mask, mask=mask)

    def training_step(self, batch, batch_idx):
        latent, label, pos, mask = batch
        model_kwargs = {'y': label, 'pos': pos, 'mask': mask, 'enable_mask':True}
    ...
    def validation_step(self, batch, batch_idx):
        latent, label, pos, mask = batch
        model_kwargs = {'y': label, 'pos': pos, 'mask': mask, 'enable_mask':False}
    ...
    def predict_step(self, x: Tensor, t: Tensor, y: Tensor, pos: Tensor, mask: Tensor, cfg_scale: Union[float, Tensor]):
            ...
            model_kwargs = dict(y=y, pos=pos, mask=mask, cfg_scale=15, enable_mask=False)
